...
import { listDeployments, VercelApiError } from "@/lib/vercel";
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const deployments = await listDeployments(session.vercelToken, params.id, session.vercelTeamId);
    ...
    return NextResponse.json({ ok: false, error: "list_deployments_failed", detail: String(err?.message || err) }, { status: 500 });
  }
}
