...
import { deployLatestFromGit, VercelApiError } from "@/lib/vercel";
...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const deployment = await deployLatestFromGit(session.vercelToken, params.id, session.vercelTeamId);
    ...
    return NextResponse.json({ ok: false, error: "git_deploy_failed", detail: String(err?.message || err) }, { status: 500 });
  }
}
